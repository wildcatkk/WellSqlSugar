...
namespace OrmTest
{
    public class Demo1_Queryable
    {
        ...
        private static void EasyExamples()
        {
            ...
            var getAll2 = db.Queryable<Order>().Select(x => x.CreateTime.Date).ToSugarList();
            var getAll3 = db.Queryable<Order>().Select(x =>Convert.ToDateTime(x.CreateTime)).ToSugarList();
            var getAll4 = db.Queryable<Order>().Select(x=>x.CreateTime.AddDays(1)).ToSugarList();
            var getAll5 = db.Queryable<Order>().Select(x => x.CreateTime.AddMonths(1)).ToSugarList();
            var getAll6 = db.Queryable<Order>().Select(x => SqlFunc.DateAdd(x.CreateTime, 1)).ToSugarList();
            var getAll7 = db.Queryable<Order>().Select(x => x.Name.ToString()).ToSugarList();
            var getAll8 = db.Queryable<Order>().Select(x => x.CreateTime.ToString("yyyy-MM-dd")).ToSugarList();
            var getAll9 = db.Queryable<Order>().Select(x => x.CreateTime.ToString("yyyy-MM-dd HH:mm:ss")).ToSugarList();
            var getAl20 = db.Queryable<Order>().Select(x => x.CreateTime.Year).ToSugarList();
            ...
            var getByFuns2 = db.Queryable<Order>().GroupBy(it => it.Name).Select(it => SqlFunc.AggregateDistinctCount(it.Price)).ToSugarList();
        ...
        private static void Mapper()
        {
            ...
            var list = db.Queryable<Tree>().Mapper(it => it.Parent, it => it.ParentId).ToSugarList();
            ...
            var list1 =db.Queryable<Tree>()
                                     ...
                                     .ToSugarList();
            //one to one
            var list2 = db.Queryable<OrderItemInfo>().Mapper(it => it.Order, it => it.OrderId).ToSugarList();

            //one to many
            var list3 = db.Queryable<Order>().Mapper(it => it.Items, it => it.Items.First().OrderId).ToSugarList();
            ...
            var result = db.Queryable<OrderInfo>().Take(10).Select<ViewOrder>().Mapper((itemModel, cache) =>
            {
                var allItems = cache.Get(orderList => {
                    ...
                    return db.Queryable<OrderItem>().Where(it => allIds.Contains(it.OrderId)).ToSugarList();//Execute only once
                });
        ...
        private static void NoEntity()
        {
            ...
            var list = db.Queryable<dynamic>().AS("Order").Where("Id=Id", new { id = 1 }).ToSugarList();

            var list2 = db.Queryable<dynamic>("o").AS("Order").AddJoinInfo("OrderDetail", "i", "o.Id=i.OrderId").Where("Id=Id", new { id = 1 }).Select("o.*").ToSugarList();
